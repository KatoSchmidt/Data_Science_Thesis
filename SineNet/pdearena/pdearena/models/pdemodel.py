...
class PDEModel(LightningModule):
    ...
    def validation_step(self, batch: Any, batch_idx: int, dataloader_idx: int = 0):
        out = self.evaluation_step(batch=batch, batch_idx=batch_idx, dataloader_idx=dataloader_idx)
        self.validation_step_outputs[dataloader_idx].append(out)
        return out
    
    def on_validation_epoch_end(self):
        if len(self.validation_step_outputs) > 1:
            self.evaluation_epoch_end(outputs=self.validation_step_outputs)
        self.validation_step_outputs = [[], []]

    def test_step(self, batch: Any, batch_idx: int, dataloader_idx: int = 0):
        out = self.evaluation_step(batch=batch, batch_idx=batch_idx, dataloader_idx=dataloader_idx, test=True)
        self.test_step_outputs[dataloader_idx].append(out)
        return out

    def on_test_epoch_end(self):
        self.evaluation_epoch_end(outputs=self.test_step_outputs, test=True)
        self.test_step_outputs = [[], []]

    # def validation_step(self, batch: Any, batch_idx: int, dataloader_idx: int = 0):
    #     return self.evaluation_step(batch=batch, batch_idx=batch_idx, dataloader_idx=dataloader_idx)

    # def on_validation_epoch_end(self): #, outputs: List[Any]): #validation_epoch_end
    #     if len(outputs) > 1:
    ...
    # def on_test_epoch_end(self):  #, outputs: List[Any]): #test_epoch_end
